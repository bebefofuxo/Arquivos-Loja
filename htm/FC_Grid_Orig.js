...
var FC_QtdDescritores=7;  //Alterado de 7 para 2 pois esta loja utiliza apenas cor e tamanho
var SeparadorRGBCor="|";
...
function FC_fRedefineOpcoes(IDProduto,aDados,aDistintos,iCampo,ValorCampo){
  ...
  for(var j=0;j<aDistintos[iCampo].length;j++){
    ...
    if(aDistintos[iCampo][j]!=ValorCampo){
      ...
      FC_Option.style.borderColor='#aaaaaa';
      FC_Option.style.color='#aaaaaa';
  ...
  if(iCampo==0 && aDados.length>1){
   ...
   if(iCampo==0){
    ...
    var sRGBCor=aNomeRGB[2];
    document.getElementById('CorProduto').innerHTML="Cor: <b>"+sNomeCor+"</b>&nbsp;&nbsp;";
    //document.getElementById('RGBCorProduto').style.backgroundColor='#'+sRGBCor;
    //document.getElementById('RGBCorProduto').style.width='30px';
    //document.getElementById('RGBCorProduto').innerHTML="<span style=width:30px;background-color:'#'"+sRGBCor+">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>";
    document.getElementById('TamanhoProduto').innerHTML='';
   }
   else if(iCampo==1){
    document.getElementById('TamanhoProduto').innerHTML="Tamanho: <b>"+ValorCampo+"</b>";
   }
  ...
  if(NivelSeguinte!=-1){  //habilita um nível abaixo
    ...
    for(var j=0;j<aDistintos[NivelSeguinte].length;j++){  //habilita opções do nível abaixo
      if(aDistintos[NivelSeguinte][j]!=''){
        ...
        if(bExisteOpcao){FC_Option.style.backgroundColor='#ffffff';}
        else{FC_Option.style.backgroundColor='#f2f2f2';}
  ...
  if(NivelSeguinte==-1){  //verifica o ID e o estoque do produto
    ...
    if(oSubProd.Existe){
      ...
      if(oSubProd.PrecoNum==0 && oSubProd.PrecoOri==0){
        ...
        document.getElementById('PrecoProdPrinc').innerHTML="Sem Preço Neste Item";
      ...
      if(!bSubProdSemPreco){
        ...
        if(oSubProd.PrecoNum!=oSubProd.PrecoOri){
         document.getElementById('PrecoProdPrinc').innerHTML="<div style='font-size:16px; color:#bdbdbd'>de <strike>"+FormatPrice(oSubProd.PrecoOri,'R$')+"</strike></div><div style='font-size:20px; color:#2f9a0a'>por <font color=#2f9a0a style='font-size:24px; font-weight:700'>"+FormatPrice(oSubProd.PrecoNum,'R$')+"</font></div>"+ParcelamentoJS;
         }
         else{
         document.getElementById('PrecoProdPrinc').innerHTML="<div style='font-size:20px; color:#2f9a0a'>por <font color=#2f9a0a style='font-size:24px; font-weight:700'>"+FormatPrice(oSubProd.PrecoNum,'R$')+"</font></div>"+ParcelamentoJS;
         }

        if(oSubProd.iEstoque>0){
          //oBotao.value="Comprar";
          oBotao.style.backgroundImage="url('"+sCaminhoImagens+"botcarrinho.png')";
    ...
    if(PrecoProdPaiNum!=PrecoProdPaiOri){
     document.getElementById('PrecoProdPrinc').innerHTML="<div style='font-size:16px; color:#bdbdbd'>de <strike>"+FormatPrice(PrecoProdPaiOri,'R$')+"</strike></div><div style='font-size:20px; color:#2f9a0a'>por <font color=#2f9a0a style='font-size:24px; font-weight:700'>"+FormatPrice(PrecoProdPaiNum,'R$')+"</font></div>"+ParcelamentoJS;
     }
     else{
     document.getElementById('PrecoProdPrinc').innerHTML="<div style='font-size:20px; color:#2f9a0a'>por <font color=#2f9a0a style='font-size:24px; font-weight:700'>"+FormatPrice(PrecoProdPaiNum,'R$')+"</font></div>"+ParcelamentoJS;
     }
...
function FC_fInitProd(IDProduto){
  ...
  for(var i=0;i<FC_QtdDescritores;i++){
    ...
    document.write("<table cellspacing=0 cellpadding=0 border=0><tr><td><div id=FC_idNivel_"+ i +"_"+ IDProduto +" class="+ FC_sClass +" style='margin-top:7px;'><ul class=lstColor>");
  ...
  document.write("<div style='padding:5px 0 7px 0; text-align:left;' align=left><table cellspacing=0 cellpadding=0 border=0><tr height=20><td id=RGBCorProduto></td><td id=CorProduto></td><td id=TamanhoProduto></td></tr></table></div><div style='text-align:left; position:relative;' align=left><span class='mascara-botao-comprar'></span><input type=button class=BotComprarGrid name=FC_oBotao_"+ IDProduto +" id=FC_oBotao_"+ IDProduto +" onClick='' value='' disabled></div>");
